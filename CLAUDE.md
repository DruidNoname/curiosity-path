...
```
app/breather/components/Breather/
  index.tsx          значения полей + состояние цикла + строка состояния
  plan.ts            фазы, BreathSettings и его строковая форма; plan.test.ts рядом
  useBreathCycle.ts  синтез звука и таймер
  components/BreathForm      поля и кнопка; свой config.ts
  components/BreathPresets   готовые практики; свой config.ts
```

- **Звук синтезируется Web Audio, файла нет.** На каждый сигнал создаётся пара
  осциллятор + гейн: узлы одноразовые, после `stop()` осциллятор не перезапускается.
  Громкость обязательно с плавными нарастанием и затуханием — включённый «в лоб»
  осциллятор щёлкает. Фазы различаются высотой тона (вдох выше, выдох ниже, пауза —
  между ними и тише), чтобы не смотреть на экран.
...
и `app/breather/components/Breather/plan.ts` (93 теста, 15 сьютов).
